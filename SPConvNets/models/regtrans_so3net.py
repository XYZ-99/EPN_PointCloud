...
class RegTransSO3ConvModel(nn.Module):
    def __init__(self, params):
        super(RegTransSO3ConvModel, self).__init__()
...
def build_model(opt,
    ...
    for i, block in enumerate(mlps):
        ...
        for j, dim_out in enumerate(block):
            ...
            block_type = 'inter_block' if na != 60 else 'transformer_block'
            conv_param = {
                'type': block_type,
                'args': {
                    'dim_in': dim_in,
    ...
    model = RegTransSO3ConvModel(params).to(device)
    return model
